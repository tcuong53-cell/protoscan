...
const walkResult = await walkPrototype(graph, { fileKey: FILE_KEY, maxScreens: 30, navTimeout: 8_000 });
console.log(`[gen-report] Simulator: ${walkResult.issues.length} runtime issues`);

console.log('[gen-report] Running static analysis...');
const result = await scan(file, { fileKey: FILE_KEY, additionalIssues: walkResult.issues });
